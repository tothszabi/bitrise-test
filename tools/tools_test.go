...
func TestStepmanJSONStepLibStepInfo(t *testing.T) {
	...
	require.Equal(t, nil, StepmanSetup("https://github.com/tothszabi/bitrise-test/v2-steplib"))

	info, err := StepmanStepInfo("https://github.com/tothszabi/bitrise-test/v2-steplib", "script", "0.9.0")
	require.NoError(t, err)
	require.NotEqual(t, "", info.JSON())

	// Invalid params -- Err returned, output is invalid
	info, err = StepmanStepInfo("https://github.com/tothszabi/bitrise-test/v2-steplib", "script", "2.x")
	require.Error(t, err)
}
